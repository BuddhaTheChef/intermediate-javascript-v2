...
//   var foundAnimal = magicAnimals.filter(function(animal) {
//     return animal.id === id;
//   })[0];
//   return updateAnimal.apply(foundAnimal, ["being majestic", "eating rainbows"]);
// }
...
CoinToss.prototype.flip = function(){
  var newcoin = (Math.floor(Math.random() * 2) == 0) ? 'heads' : 'tails';

  this.results.push(newcoin);
  return this.results;
}
...
function partyTime(partyName){
  ...
  return addGuest = function(guest){
     arr.push(guest);
    return {partyName: partyName, guestList:arr};
  }
}
